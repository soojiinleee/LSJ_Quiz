...
class TestAttemptQuiz:
    def test_quiz_attempt_create(
        self, api_client, user_data, quiz_data, question_data, quiz_question_data
    ):
        ...
        question_ids = [
            question_data["question2"].id,
            question_data["question3"].id,
            question_data["question1"].id,
        ]
        request_data = {
            "quiz_id": quiz_id,
    ...
    def test_already_attempted_quiz(
        self, api_client, user_data, quiz_data, question_data, quiz_attempt_data
    ):
        """응시 이력 있는 퀴즈 재응시 테스트"""
...
@pytest.mark.django_db
class TestAttemptQuestionWithChoice:

    def test_attempt_question_detail_random_choice(
        self,
        api_client,
        user_data,
        quiz_data,
        ...
        quiz_question_data,
        quiz_attempt_data,
        quiz_attempt_question_data,
        quiz_attempt_choice_data,
    ):
        """응시 문제 상세 조회 : 선택지 랜덤 정렬"""
    ...
    def test_attempt_question_detail_with_ordered_choice(
        ...
        # then : 선택지 정렬 확인
        response_data = response.json()
        expected_order = [choice_data["choice5"].id, choice_data["choice4"].id]
...
@pytest.mark.django_db
class TestAttemptChoice:
    def test_save_quiz_attempt_choice_order(
        self,
        api_client,
        user_data,
        quiz_data,
        ...
        question_id = quiz_attempt_question_data["attempt1_question3"].question.id
        choice_ids = [
            choice_data["choice9"].id,
            choice_data["choice8"].id,
            choice_data["choice7"].id,
            choice_data["choice6"].id,
        ]
        request_data = {
            "quiz_id": quiz_id,
        ...
        # then : 선택지 저장 성공 확인
        stored = QuizAttemptChoice.objects.filter(
            attempt_question=quiz_attempt_question_data["attempt1_question3"]
        )
    ...
    def test_saved_selected_choice(
        self,
        api_client,
        user_data,
        quiz_data,
    ...
    def test_update_selected_choice(
        self,
        api_client,
        user_data,
        quiz_data,
    ...
    def test_quiz_submission(
        ...
        corrected_question = QuizAttemptQuestion.objects.get(
            attempt=quiz_attempt_data["attempt1"], question=question_data["question2"]
        )
