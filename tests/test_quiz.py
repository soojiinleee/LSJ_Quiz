...
class TestUpdateQuiz:
    def test_update_quiz_question_count(
        self, api_client, staff_user_data, quiz_data, quiz_question_data
    ):
        """퀴즈 문제 개수 설정 (0 -> 2)"""
    ...
    def test_fail_update_quiz_question_count(
        self, api_client, staff_user_data, quiz_data, quiz_question_data
    ):
        ...
        assert (
            response.json()["question_count"][0]
            == "문제 수는 현재 연결된 문제 수(0 개)를 초과할 수 없습니다."
        )

    def test_update_quiz_question_random_order(
...
class TestReadQuiz:
    ...
    def test_quiz_list_pagination(
        self, api_client, staff_user_data, quiz_pagination_data
    ):
        """퀴즈 목록 페이지네이션 테스트"""
    ...
    def test_quiz_list_by_user(
        self, api_client, user_data, quiz_data, quiz_attempt_data
    ):
        """일반 사용자 : 퀴즈 목록 조회 테스트"""
...
class TestQuizQuestionLink:
    ...
    def test_link_to_quiz_with_not_existed_question(
        self, api_client, quiz_data, question_data
    ):
...
class TestQuizQuestionList:
    ...
    def test_quiz_question_list(
        self, api_client, user_data, quiz_with_questions_batch_data
    ):
    ...
    def test_quiz_attempt_question_list(
        self,
        api_client,
        user_data,
        quiz_data,
        ...
        ids = [
            aq.question.id
            for aq in sorted(attempt_questions, key=lambda x: x.order_index)
        ]
